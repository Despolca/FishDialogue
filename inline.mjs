...
export function tokenHTML(html) {
    return html.replace(/⟦FAP(\d+|WAIT|BAD)⟧/g,(_,id) => {
        if (id === 'WAIT' || id === 'BAD') return `<span class="fa-inline-note">${id === 'WAIT' ? 'Đang chuẩn bị âm thanh...' : 'Lỗi định dạng âm thanh'}</span>`;
        return `<span class="fa-inline" data-fa-block="${id}"><button type="button" class="fa-line-play" aria-label="Phát câu này">▶ Phát</button><button type="button" class="fa-line-replay" aria-label="Phát lại câu này">↺</button><span class="fa-line-state">Đoạn ${Number(id) + 1}</span></span>`;
    });
}
function fragment(text, doc) {
    ...
    for (const m of text.matchAll(/⟦FAP(\d+|WAIT|BAD)⟧/g)) {
        ...
        if (m[1] === 'WAIT' || m[1] === 'BAD') {
            span.className = 'fa-inline-note'; span.textContent = m[1] === 'WAIT' ? 'Đang chuẩn bị âm thanh...' : 'Lỗi định dạng âm thanh';
        } else {
            span.className = 'fa-inline'; span.dataset.faBlock = m[1];
            for (const [name,label,aria] of [['play','▶ Phát','Phát câu này'],['replay','↺','Phát lại câu này']]) {
...
export function reconcileControls(element, target, records, language) {
    ...
    if (!dock) {
        ...
        dock.setAttribute('aria-label','Âm thanh từng câu của tin nhắn này'); target.after(dock);
    }
    ...
    function render() {
        ...
        for (const element of document.querySelectorAll('#chat .mes[mesid]')) {
            ...
            for (const control of element.querySelectorAll('.fa-inline[data-fa-block]')) {
                ...
                const label=muted ? 'Đã chặn' : active && state.running ? (state.phase==='generating'?'Đang đợi âm thanh...':state.paused?'▶ Tiếp tục':'❚❚ Tạm dừng') : '▶ Phát';
                if (button.textContent!==label) button.textContent=label;
                const suffix=p ? ({queued:'Chờ tổng hợp',requesting:'Đang yêu cầu',receiving:'Đang nhận',saving:'Đang lưu',ready:'Đã sẵn sàng',failed:'Thất bại',canceled:'Đã hủy'}[p.status] || '') : '';
