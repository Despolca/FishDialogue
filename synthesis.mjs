...
export class SynthesisQueue {
    ...
    submit(item, { retainAudio = true } = {}) {
        ...
        if (this.pending.length >= 100) throw new Error('Hàng đợi tổng hợp đã đầy (100 câu)');
    ...
    cancelTask(task) {
        ...
        task.controller?.abort(); task.status = 'canceled'; task.error = 'Đã hủy'; task.finishedAt = Date.now();
    ...
    async take(task, signal) {
        ...
        if (signal.aborted) throw new DOMException('Aborted','AbortError');
        if (task.status !== 'ready') throw new Error(task.error || 'Tổng hợp chưa hoàn thành');
        const blob = task.blob;
        if (!blob) throw new Error('Âm thanh đã được phát hoặc giải phóng, vui lòng nhấp phát lại');
