...
export function resolveAccountRoot(req) {
    ...
    if (typeof supplied !== 'string' || !supplied.trim() || supplied.includes('\0')) {
        throw new Error('Yêu cầu từ tửu quán không cung cấp thư mục tài khoản hợp lệ (req.user.directories.root); vui lòng xác nhận đã đăng nhập và kiểm tra nhật ký máy chủ');
    }
...
export class AudioStore {
    ...
    transaction(fn) {
        const job = this.tail.catch(() => {}).then(async () => {
            ...
            if (path.dirname(dir) !== root) throw new Error('Thư mục âm thanh không được là liên kết trỏ ra ngoài thư mục tài khoản');
            let state;
            try { state = JSON.parse(await fs.readFile(path.join(dir, 'library.json'), 'utf8')); }
            catch (e) { if (e.code !== 'ENOENT') throw new Error('Chỉ mục âm thanh bị hỏng, vui lòng sao lưu thư mục rồi sửa chữa library.json'); state = { maxFiles: 200, entries: [] }; }
            if (!Array.isArray(state.entries) || !Number.isInteger(state.maxFiles) || state.maxFiles < 1 || state.maxFiles > 10000) throw new Error('Định dạng chỉ mục âm thanh bị lỗi');
            return fn(state, dir);
        });
    ...
    async trim(state, dir) {
        ...
        while (state.entries.length > state.maxFiles) {
            ...
            if (!validId(entry.id)) throw new Error('Chỉ mục âm thanh không hợp lệ');
    ...
    read(id) {
        if (!validId(id)) throw new Error('ID âm thanh không hợp lệ');
        return this.transaction(async (state, dir) => {
            if (!state.entries.some(e => e.id === id)) return null;
            const filename = path.join(dir, id + '.mp3');
            try {
                const st = await fs.lstat(filename);
                if (!st.isFile() || st.isSymbolicLink()) throw new Error('Âm thanh không phải là tệp thông thường');
    ...
    save(id, bytes, input) {
        ...
        return this.transaction(async (state, dir) => {
            ...
            const entry = { id, createdAt: old?.createdAt || Math.max(Date.now(), ...state.entries.map(e => e.createdAt + 1)), bytes: bytes.length,
                speaker: String(input.speaker || 'mặc định').slice(0, 100), language: String(input.language || 'orig').slice(0, 12),
    ...
    setLimit(value) {
        if (!Number.isInteger(value) || value < 1 || value > 10000) throw new Error('Số lượng giữ lại tối đa phải là số nguyên từ 1–10000');
        return this.transaction(async (state, dir) => {
...
export async function openAudioFolder(req, directory) {
    ...
    if (!['127.0.0.1', '::1', '::ffff:127.0.0.1'].includes(remote)) throw new Error('Không thể mở desktop máy chủ khi truy cập từ xa; vui lòng sử dụng đường dẫn thư mục được hiển thị hoặc tải xuống âm thanh');
    const commands = { win32: ['explorer.exe', [directory]], darwin: ['open', [directory]], linux: ['xdg-open', [directory]] };
    const command = commands[process.platform];
    if (!command) throw new Error('Hệ thống hiện tại không hỗ trợ tự động mở thư mục');
